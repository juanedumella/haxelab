...
class CombatHUD extends FlxTypedGroup<FlxSprite> {
	...
	public function new() {
		...
		_sprBack.drawRect(1, 1, 198, 50, FlxColor.BLACK); // Cuadro Negro 1	
		_sprBack.drawRect(1, 50, 198, 69, FlxColor.BLACK); // Cuadro Negro 2
		...
		_choices.push(new FlxText(_sprBack.x + 30, _sprBack.y + 48, -1, "COMBATIR", 14));
		
		_choices.push(new FlxText(_sprBack.x + 30, _choices[0].y + _choices[0].height +  8, -1, "HUIR", 14));
		add(_choices[0]);
		add(_choices[1]);
		
		_pointer = new FlxSprite(_sprBack.x + 10, _choices[0].y + (_choices[0].height / 2) - 8, AssetPaths.manito__png); // Puntero
	...
	public function initCombat(PlayerHealth:Int, E:Enemy):Void
	{
		...
		FlxTween.num(0, 1, .33, { ease:FlxEase.circOut, complete:finishFadeIn }, updateAlpha);	// do a numeric tween to fade in our combat hud when the tween is finished, call finishFadeIn
